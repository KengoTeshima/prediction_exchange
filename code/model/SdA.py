...
class DA:
    ...
    def train_and_test(self, n_epoch=5, batchsize=40):
...
class SDA:
    def __init__(
        ...
        self.model = Chain(l1=L.Linear(n_inputs, n_hidden[0]),
                                 ...
                                 l4=L.Linear(n_hidden[2], n_outputs))
    ...
    def forward(self, x_data, y_data, train=False, output=False):
        x, t = Variable(x_data), Variable(y_data)
        h1 = F.dropout(F.relu(self.model.l1(x)), train=train)
        h2 = F.dropout(F.relu(self.model.l2(h1)), train=train)
        h3 = F.dropout(F.relu(self.model.l3(h2)), train=train)
        y = F.tanh(self.model.l4(h3))
        if output:
            return y
        else:
    ...
    def fine_tune(self, n_epoch=20, batchsize=50):
        ...
        #早期終了用配列
        self.save_accuracy = self.xp.tile([1000.0],50)
